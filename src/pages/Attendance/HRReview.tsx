...
import { CheckCircle2, AlertCircle, Users, UserMinus, Briefcase, Send, ListTodo } from 'lucide-react';
...
import { getDailyApprovalStatus, submitForDirectorApproval, type ApprovalStatus } from '../../lib/mockApproval';
...
export default function HRReview() {
    ...
    const handleSubmitForApproval = () => {
        if (confirm(`ยืนยันการส่งข้อมูลลงเวลาของวันที่ ${moment(currentDate).format('DD/MM/YYYY')} ให้ผู้อำนวยการอนุมัติ?`)) {
            submitForDirectorApproval(currentDate);
            setApprovalStatus('HR_SUBMITTED');
        }
    };
        ...
        <div className="max-w-5xl mx-auto space-y-6">
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 py-2 mb-6">
                <div className="flex items-center gap-4">
                    <div className="bg-gradient-to-tr from-amber-500 to-orange-500 p-3 rounded-xl shadow-md shadow-orange-500/20">
                        <ListTodo className="w-8 h-8 text-white" />
                    </div>
                    <div>
                        <h1 className="text-2xl font-bold bg-gradient-to-r from-gray-900 to-gray-700 bg-clip-text text-transparent">ตรวจสอบข้อมูลการลงเวลา (HR)</h1>
                        <p className="text-sm text-gray-500">ตรวจสอบและส่งให้ผู้อำนวยการอนุมัติ</p>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm flex flex-col md:flex-row gap-4 items-center justify-between">
                ...
                <div className="flex items-center gap-3">
                    <div className="text-sm font-semibold mr-2">
                        สถานะ :
                        {approvalStatus === 'PENDING' && <span className="ml-2 text-gray-500 bg-gray-100 px-3 py-1 rounded-full">รอดำเนินการ</span>}
                    ...
                    <button
                        ...
                        className={`flex items-center gap-2 px-4 py-2 rounded-xl font-semibold transition-all ${approvalStatus === 'PENDING'
